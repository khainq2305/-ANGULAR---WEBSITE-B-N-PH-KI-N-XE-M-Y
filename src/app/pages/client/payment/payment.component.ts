...
onDistrictChange() {
  this.shippingService.getWards(this.selectedDistrictId).subscribe(
    res => this.wards = res.data 
  );
}
      ...
      const tryFee = (index: number) => {
        ...
        this.shippingService.calculateFee({
          ...
          next: (res) => {
            ...
            this.shippingFee = res.data.total;
          },
          error: () => {
...
onPlaceOrder() {
  ...
  const payload = {
    ...
    paymentMethod: 'COD',                     
    totalPrice: this.totalAmount + this.shippingFee, 
    shippingMethod: 'GHN',                     
