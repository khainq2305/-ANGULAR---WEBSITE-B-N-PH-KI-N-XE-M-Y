      ...
      next: (res) => {
        ...
        if (user?.id) {
          this.cartService.getCartByUser(user.id).subscribe((cartRes: any) => {
            ...
            const productInCart = items.find((item: any) => item.product_id === this.product.id);
...
if (maxAddable <= 0) {
  ...
  onImageError(event: any) {
    event.target.src = 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT5-I3nwE8w_QXqUKIaA9R5Rjr-l7UOVLdPWQ&s';
  }
  ...
  addToCart() {
    ...
    this.productService.addToCart({ product_id, quantity: this.quantity }).subscribe({
      next: () => {
        ...
        if (user?.id) {
          this.cartService.getCartByUser(user.id).subscribe((cartRes: any) => {
            ...
            this.cartService.setCartItems(items); // ✅ cập nhật lên header
          });
        }
      ...
      error: (err) => {
        this.errorMessage = err.error?.message || 'Thêm vào giỏ hàng thất bại';
